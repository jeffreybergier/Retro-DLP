...
- (void)start {
  @try {
    ...
    Check([_queue currentIndex]==1 && [[controls nextButton] isEnabled],@"UI previous request changes the real queue and updates availability");
    [[[controls nextButton] target] performSelector:[[controls nextButton] action] withObject:[controls nextButton]];
    Check([_queue currentIndex]==2 && ![[controls nextButton] isEnabled],@"UI next request reaches the playlist end");
    [[[controls audioButton] target] performSelector:[[controls audioButton] action] withObject:[controls audioButton]];
    Check([_queue isAudioOnly] && [_view isAudioOnly],@"Audio-only request updates queue and UI together");
    ...
    Check([_queue isAudioOnly],@"Audio preference survives playlist replacement");
    ...
    [_view setDelegate:nil]; [_view release]; _view=nil;
...
- (void)queueChanged:(NSNotification *)notification {
  ...
  [_view setCanSkipToPreviousItem:[_queue canSkipToPreviousItem]];
