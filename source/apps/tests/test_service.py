...
subprocess.run(['cc', '-std=c99', '-Wall', '-Wextra', '-Werror',
    '-I'+str(ROOT/'include'), '-I'+str(ROOT/'source/apps/shared'),
    str(ROOT/'source/apps/tests/service_test.c'),
    str(ROOT/'source/apps/shared/rdapp_store.c'), str(ROOT/'source/apps/shared/rdapp_service.c'),
...
with tempfile.TemporaryDirectory(prefix='rdapp-service-') as directory:
    ...
    server=subprocess.Popen(['python3',str(ROOT/'source/linux/test/download_server.py'),str(certificate),str(key),str(port)])
    try:
        for _ in range(100):
